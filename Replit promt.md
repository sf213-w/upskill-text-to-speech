...
## What We're Building
"Care Clarity" is a bilingual (Spanish ↔ English) medical translation web app (PWA) that helps Spanish-speaking patients communicate with English-speaking doctors in real time. This is a prototype/demo MVP — focused on proving the core idea works, not on production compliance.
...
- Upgrade to Azure AI Translator for the Pilot phase
...
- Language toggle (ES | EN) on every screen — top right — switches entire UI instantly
- ALL strings, buttons, errors in Spanish by default
- Large tap targets (min 48px), voice input as primary interaction

### 2. Simple Onboarding (3 Screens)
- Screen 1: Welcome in Spanish — "Bienvenido a Care Clarity" + "Comenzar" button
- Screen 2: Microphone permission request — explain why in Spanish
- Screen 3: One-line privacy notice — "🔒 Este app no guarda grabaciones de audio."
...
- Google Translate API translates to English
...
- Google Translate API translates to Spanish
...
- "Terminar sesión / End Session" button at the bottom
...
- "Guardar / Save" → saves to localStorage
- "Compartir / Share" → uses Web Share API
- "Historial / History" tab shows all saved sessions
- Sessions deletable from History screen

### 5. Provider Search (Hardcoded Placeholder)
- Search bar in Spanish: "¿Cuál es tu preocupación de salud?"
- Accepts voice or text input
- Query a hardcoded `providers.json` file of 8–10 sample Spanish-speaking providers
- Display result cards: name, specialty, location, languages spoken — all in Spanish
- "Llamar / Call" button (tel: link)
- "Reservar cita / Book Appointment" button (disabled placeholder — "Próximamente")
...
- Bottom navigation — 4 tabs:
  - 🏠 Inicio (Home/Welcome)
  - 🎙️ Consulta (Consultation)
  - 📋 Historial (History)
  - ⚙️ Ajustes (Settings)
- Privacy badge visible on Home: "🔒 Sin almacenamiento de audio / No audio stored"
- All error messages appear in Spanish first
...
2. i18next setup — wire `es.json` / `en.json` + language toggle
3. Express server + `/api/translate` route (Google Translate)
...
/care-clarity
  ...
  package.json
Build Order
Vite + React PWA + Tailwind setup
i18next setup — wire en.json / es.json + language toggle (default: English)
Express server + /api/translate route (MyMemory API)
Consultation screen — Web Speech API (both panels) + translation + TTS
Transcript save/view (localStorage)
Bottom navigation + screen routing
Onboarding flow (3 screens)
Provider search (hardcoded JSON)
